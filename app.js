...
// ============================================================================
// 1-1. 보안 및 인증 헬퍼 함수 (Web Crypto API SHA-256 & 비밀번호 정책)
// ============================================================================
...
// 비밀번호 정책 검증: 8자 이상, 숫자 1개 이상 필수 포함
...
const DEFAULT_ADMINS = [
  { id: "admin_super", username: "admin", password: "1q2w3e4r!", name: "대표 최고관리자", role: "super", createdAt: Date.now() - 86400000 * 30 },
  { id: "admin_gen1", username: "manager1", password: "1q2w3e4r!", name: "김상담 실장", role: "general", createdAt: Date.now() - 86400000 * 10 }
];
...
function initLocalStorage() {
  ...
  if (!localStorage.getItem("crm_admins")) {
    localStorage.setItem("crm_admins", JSON.stringify(DEFAULT_ADMINS));
  }
...
async function initFirebaseData() {
  ...
  try {
    ...
    const adminsSnap = await window.db.collection("admins").get();
    if (adminsSnap.empty) {
      ...
      console.log("🔥 Firebase: 기본 관리자 계정 데이터를 초기화했습니다.");
    }
...
async function dbGetAdmins() {
  if (window.isFirebaseMode) {
    try {
      const snap = await window.db.collection("admins").orderBy("createdAt", "asc").get();
    ...
    return JSON.parse(localStorage.getItem("crm_admins") || "[]");
  }
}

async function dbAddAdmin(username, password, name, role = "general") {
  const cleanUsername = username.trim();
  const cleanName = name.trim();
  const cleanPassword = password.trim();

  if (window.isFirebaseMode) {
    const docRef = await window.db.collection("admins").add({
...
async function dbUpdateAdmin(id, name, role, newPassword = null) {
  const cleanName = name.trim();
  const updateData = {
    name: cleanName,
    role: role,
  ...
  if (window.isFirebaseMode) {
    await window.db.collection("admins").doc(id).update(updateData);
  } else {
    const admins = await dbGetAdmins();
...
async function dbDeleteAdmin(id) {
  if (window.isFirebaseMode) {
    await window.db.collection("admins").doc(id).delete();
  } else {
...
const DOM = {
  ...
  statSuperAdmins: document.getElementById("stat-super-admins"),
  statGeneralAdmins: document.getElementById("stat-general-admins"),
  searchAdmin: document.getElementById("search-admin"),
  adminListTableBody: document.getElementById("admin-list-table-body"),
  btnOpenAdminRegisterModal: document.getElementById("btn-open-admin-register-modal"),
  ...
  // 관리자 등록 모달
  adminRegisterModal: document.getElementById("admin-register-modal"),
  formRegisterAdmin: document.getElementById("form-register-admin"),
  adminRegUsername: document.getElementById("admin-reg-username"),
  adminRegPassword: document.getElementById("admin-reg-password"),
  adminRegName: document.getElementById("admin-reg-name"),
  adminRegRole: document.getElementById("admin-reg-role"),
  btnCloseAdminRegisterOverlay: document.getElementById("btn-close-admin-register-overlay"),

  // 관리자 정보 수정 모달
  adminEditModal: document.getElementById("admin-edit-modal"),
  formEditAdmin: document.getElementById("form-edit-admin"),
  adminEditId: document.getElementById("admin-edit-id"),
  adminEditUsername: document.getElementById("admin-edit-username"),
  adminEditName: document.getElementById("admin-edit-name"),
  adminEditRole: document.getElementById("admin-edit-role"),
  adminEditPassword: document.getElementById("admin-edit-password"),
  adminEditPasswordConfirm: document.getElementById("admin-edit-password-confirm"),
  adminEditPwMatch: document.getElementById("admin-edit-pw-match"),
  btnCloseAdminEditOverlay: document.getElementById("btn-close-admin-edit-overlay"),
...
function renderView(viewId) {
  ...
  if (viewId === "client-login") {
    ...
    // 현재 세션 관리자 권한 UI 반영 (nav className 할당 이후에 실행하여 덮어쓰기 방지)
    const adminUser = getCurrentAdminUser();
    applyAdminPermissions(adminUser);
...
function getCurrentAdminUser() {
  try {
    const stored = sessionStorage.getItem("crm_admin_user");
    if (stored) return JSON.parse(stored);
  } catch (e) {
    console.error(e);
  }
  return { username: "admin", name: "최고 관리자", role: "super" };
}

// 관리자 권한별 UI 제어 (최고 관리자 / 일반 관리자)
function applyAdminPermissions(adminUser) {
  if (!adminUser) return;
  const isSuper = adminUser.role === "super";

  // 헤더 사용자 및 역할 뱃지 표시
  if (DOM.adminUserDisplay) {
    DOM.adminUserDisplay.textContent = adminUser.name || adminUser.username;
  }
  if (DOM.adminRoleBadge) {
    DOM.adminRoleBadge.textContent = isSuper ? "최고 관리자" : "일반 관리자";
    DOM.adminRoleBadge.className = isSuper 
      ? "text-[10px] px-1.5 py-0.5 rounded bg-purple-100 text-purple-700 font-medium" 
      : "text-[10px] px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 font-medium";
  }

  // 1. 상담지 관리 메뉴: 일반 관리자는 숨김
  if (DOM.navCounseling) {
    if (isSuper) {
      DOM.navCounseling.classList.remove("hidden");
      DOM.navCounseling.style.display = "";
    } else {
      DOM.navCounseling.classList.add("hidden");
      DOM.navCounseling.style.display = "none";
    }
  }
  ...
  if (DOM.navManagers) {
    if (isSuper) {
      ...
      DOM.navManagers.style.display = "";
    } else {
      DOM.navManagers.classList.add("hidden");
      DOM.navManagers.style.display = "none";
    }
  }
  ...
  if (DOM.btnOpenRegisterModal) {
    if (isSuper) {
      ...
      DOM.btnOpenRegisterModal.style.display = "";
    } else {
      DOM.btnOpenRegisterModal.classList.add("hidden");
      DOM.btnOpenRegisterModal.style.display = "none";
    }
  }
...
async function handleAdminLogin(e) {
  ...
  if (matchedAdmin) {
    ...
    sessionStorage.setItem("crm_admin_user", JSON.stringify(matchedAdmin));
    applyAdminPermissions(matchedAdmin);
    showToast(`${matchedAdmin.name}(${matchedAdmin.role === "super" ? "최고 관리자" : "일반 관리자"}) 로그인 완료`, "success");
...
async function renderAdminDirectory() {
  if (!DOM.adminListTableBody) return;
  const admins = await dbGetAdmins();
  const currentAdmin = getCurrentAdminUser();
  const queryText = DOM.searchAdmin ? DOM.searchAdmin.value.toLowerCase().trim() : "";

  // 지표 업데이트
  if (DOM.statTotalAdmins) DOM.statTotalAdmins.textContent = admins.length;
  if (DOM.statSuperAdmins) DOM.statSuperAdmins.textContent = admins.filter(a => a.role === "super").length;
  if (DOM.statGeneralAdmins) DOM.statGeneralAdmins.textContent = admins.filter(a => a.role === "general").length;
  ...
  if (filtered.length === 0) {
    DOM.adminListTableBody.innerHTML = `
      <tr>
        <td colspan="4" class="py-md px-md text-center text-on-surface-variant/40">
          등록된 관리자 계정이 없습니다.
        </td>
      </tr>
    `;
  ...
  filtered.forEach(admin => {
    ...
    const roleBadge = isSuper
      ? `<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-purple-100 text-purple-700 border border-purple-200">
           <span>👑</span> 최고 관리자
         </span>`
    ...
    const regDateStr = new Date(admin.createdAt || Date.now()).toLocaleDateString("ko-KR", {
    ...
    tr.innerHTML = `
      <td class="py-md px-md">
        <div class="flex items-center gap-sm">
          <div class="w-8 h-8 rounded-full ${isSuper ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'} flex items-center justify-center font-bold text-xs flex-shrink-0">
            ${admin.name ? admin.name.slice(0, 2) : '관'}
          </div>
          <div class="flex flex-col">
            <span class="font-semibold text-on-surface">${admin.name}</span>
            <span class="text-xs text-on-surface-variant/70 font-mono">아이디: ${admin.username}</span>
          </div>
        </div>
      </td>
      <td class="py-md px-md">${roleBadge}</td>
      <td class="py-md px-md text-on-surface-variant">${regDateStr}</td>
      <td class="py-md px-md text-right">
        <div class="flex items-center justify-end gap-xs">
          <button class="btn-edit-admin px-3 py-1.5 rounded-lg border border-outline-variant/50 text-on-surface hover:bg-surface-container-high font-label-sm text-xs transition-colors flex items-center gap-1" data-id="${admin.id}">
            <span class="material-symbols-outlined text-[14px]">settings</span>
            설정
          </button>
          <button class="btn-delete-admin px-3 py-1.5 rounded-lg border border-error/30 text-error hover:bg-error-container/20 font-label-sm text-xs transition-colors flex items-center gap-1" data-id="${admin.id}">
    ...
    // 삭제 버튼 이벤트
    tr.querySelector(".btn-delete-admin").addEventListener("click", async () => {
      if (currentAdmin && currentAdmin.username === admin.username) {
        showToast("현재 로그인 중인 본인 계정은 삭제할 수 없습니다.", "error");
        return;
      }

      const superAdmins = admins.filter(a => a.role === "super");
      if (admin.role === "super" && superAdmins.length <= 1) {
        showToast("최소 1명 이상의 최고 관리자가 존재해야 하므로 삭제할 수 없습니다.", "error");
        return;
      }

      if (!confirm(`'${admin.name}(${admin.username})' 관리자 계정을 정말로 삭제하시겠습니까?`)) {
        return;
      }

      await dbDeleteAdmin(admin.id);
      showToast("관리자 계정이 삭제되었습니다.");
      renderAdminDirectory();
    });

    DOM.adminListTableBody.appendChild(tr);
  });
}

function openAdminEditModal(admin) {
  DOM.adminEditId.value = admin.id;
  DOM.adminEditUsername.value = admin.username;
  DOM.adminEditName.value = admin.name;
  DOM.adminEditRole.value = admin.role || "general";
  DOM.adminEditPassword.value = "";
  if (DOM.adminEditPasswordConfirm) DOM.adminEditPasswordConfirm.value = "";
  if (DOM.adminEditPwMatch) {
    DOM.adminEditPwMatch.classList.add("hidden");
    DOM.adminEditPwMatch.textContent = "";
  }
...
async function renderClientDirectory() {
  ...
  if (filtered.length === 0) {
    DOM.clientListTableBody.innerHTML = `
      <tr>
        <td colspan="7" class="py-md px-md text-center text-on-surface-variant/40">
          검색된 내담자 정보가 없습니다.
        </td>
      </tr>
    `;
  ...
  filtered.forEach(client => {
    ...
    if (phoneStr !== "-" && !phoneStr.includes("-")) {
      phoneStr = phoneStr.replace(/(\d{3})(\d{3,4})(\d{4})/, "$1-$2-$3");
    }
...
function registerEventListeners() {
  ...
  // 관리자 등록 모달 열기/닫기
  if (DOM.btnOpenAdminRegisterModal) {
    DOM.btnOpenAdminRegisterModal.addEventListener("click", () => {
      DOM.formRegisterAdmin.reset();
      DOM.adminRegisterModal.classList.remove("hidden");
    });
  }

  const closeAdminRegisterModal = () => {
    if (DOM.adminRegisterModal) DOM.adminRegisterModal.classList.add("hidden");
  };
  document.querySelectorAll(".btn-close-admin-register").forEach(btn => btn.addEventListener("click", closeAdminRegisterModal));
  if (DOM.btnCloseAdminRegisterOverlay) {
    DOM.btnCloseAdminRegisterOverlay.addEventListener("click", closeAdminRegisterModal);
  }

  // 관리자 등록 제출
  if (DOM.formRegisterAdmin) {
    DOM.formRegisterAdmin.addEventListener("submit", async (e) => {
      if (e) e.preventDefault();
      const username = DOM.adminRegUsername.value.trim();
      const password = DOM.adminRegPassword.value.trim();
      const name = DOM.adminRegName.value.trim();
      const role = DOM.adminRegRole.value;
      ...
      if (admins.some(a => a.username.toLowerCase() === username.toLowerCase())) {
        showToast("이미 존재하는 관리자 아이디입니다.", "error");
        return;
      }

      await dbAddAdmin(username, password, name, role);
      closeAdminRegisterModal();
      showToast(`'${name}' 관리자 계정이 등록되었습니다.`);
      renderAdminDirectory();
    });
  }
  ...
  const closeAdminEditModal = () => {
    if (DOM.adminEditModal) DOM.adminEditModal.classList.add("hidden");
  };
  document.querySelectorAll(".btn-close-admin-edit").forEach(btn => btn.addEventListener("click", closeAdminEditModal));
  if (DOM.btnCloseAdminEditOverlay) {
    DOM.btnCloseAdminEditOverlay.addEventListener("click", closeAdminEditModal);
  }

  // 관리자 정보 수정 모달 비밀번호 일치 실시간 검사
  function checkAdminEditPwMatch() {
    if (!DOM.adminEditPassword || !DOM.adminEditPasswordConfirm || !DOM.adminEditPwMatch) return;
    const pw = DOM.adminEditPassword.value;
    const pwConfirm = DOM.adminEditPasswordConfirm.value;
    ...
    DOM.adminEditPwMatch.classList.remove("hidden");
    if (pw === pwConfirm) {
      DOM.adminEditPwMatch.className = "font-body-sm text-xs text-primary mt-1";
      DOM.adminEditPwMatch.textContent = "✓ 새 비밀번호가 일치합니다.";
    } else {
      DOM.adminEditPwMatch.className = "font-body-sm text-xs text-error mt-1";
      DOM.adminEditPwMatch.textContent = "✕ 새 비밀번호가 일치하지 않습니다.";
    }
  }

  if (DOM.adminEditPassword) DOM.adminEditPassword.addEventListener("input", checkAdminEditPwMatch);
  if (DOM.adminEditPasswordConfirm) DOM.adminEditPasswordConfirm.addEventListener("input", checkAdminEditPwMatch);

  // 관리자 정보 수정 제출
  if (DOM.formEditAdmin) {
    DOM.formEditAdmin.addEventListener("submit", async (e) => {
      if (e) e.preventDefault();
      const id = DOM.adminEditId.value;
      const name = DOM.adminEditName.value.trim();
      const role = DOM.adminEditRole.value;
      const newPassword = DOM.adminEditPassword.value.trim();
      const newPasswordConfirm = DOM.adminEditPasswordConfirm ? DOM.adminEditPasswordConfirm.value.trim() : "";
      ...
      // 마지막 최고 관리자의 등급 강등 방지
      if (targetAdmin && targetAdmin.role === "super" && role === "general") {
        const superAdmins = admins.filter(a => a.role === "super");
        if (superAdmins.length <= 1) {
          showToast("최소 1명의 최고 관리자가 유지되어야 하므로 일반 관리자로 변경할 수 없습니다.", "error");
          return;
        }
      }

      await dbUpdateAdmin(id, name, role, newPassword || null);

      // 만약 본인 계정이 수정되었으면 세션 업데이트
      const currentAdmin = getCurrentAdminUser();
      if (currentAdmin && currentAdmin.username === targetAdmin.username) {
        currentAdmin.name = name;
        currentAdmin.role = role;
        sessionStorage.setItem("crm_admin_user", JSON.stringify(currentAdmin));
        applyAdminPermissions(currentAdmin);
      }
